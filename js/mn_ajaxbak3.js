...
{
window.alert('update go button submitted_category='+submitted_category+', submitted_regional_num = '+submitted_regional_num+', nonce = '+nonce+', original_cat_id ='+original_cat_id);
...
function showSubLoc1(str, main_cat_nonce,currentLevel,cat_id) 
{
/*
Process:
...
var output = '<form action=""><select name="subLoc1" onchange="showSubLoc2(this.value,\''+main_cat_nonce+'\',\''+nextLevel+'\'),updategoButton(\'false\',this.value,\''+main_cat_nonce+' \',\''+myarr[1]+' \',\''+'\',\''+cat_id+'\')"><option value="">' + wording_ajax_menu1 + '</option>';

//+'\',\''+cat_id+'\')
//(submitted_category, submitted_regional_num, nonce, original_cat_id)
...
window,alert('showSubLoc'+currentLevel+' func'+output+'<div class="locHint'+nextLevel+'" id="locHint'+nextLevel+'" name="locHint'+nextLevel+'" >'+still_more_cats+'</div>');
      ...
      document.getElementById("locHint1").innerHTML=output+'<div class="locHint2" id="locHint2" name="locHint2" >'+still_more_cats+'</div>';
     }
 }
  }
