...
window.alert('category_id ='+category_id+',main_cat_nonce = '+main_cat_nonce+'original_cat_id = '+original_cat_id);
alert('document.getElementById("clearbutton") statement = deleteAllLevels(\''+category_id+'\',\''+main_cat_nonce+'\',\''+original_cat_id+'\')');
...
window.alert('The updategoButton call sent by incl_regional_wp.php = submitted_category, submitted_regional_num, nonce, original_cat_id'+submitted_category+','+ submitted_regional_num+','+ nonce, original_cat_id);			
...
alert('in if a = -1 created mycatarr = '+mycatarr);
}
else
{
var mycatarr = submitted_category.split(":");
alert('in else of if a = -1 submitted.category mycatarr = '+mycatarr);
...
alert('in else of if submitted_regional_num === \'false\' = '+inserturl);
...
window.alert('region_str = '+region_str);
...
/* backup
function deleteAllLevels(category_id,main_cat_nonce,original_cat_id) {	
//we need to be careful to use the right "category_id". We need to use the original_category_id for the page that they are currently looking at the results of and the submenu to be displayed will be of that. This function needs to pass that viewed category id and not the one passed by the dropdown selection. This function basically removes the entire center div (with the Go and Clear buttons that appear AFTER a selection) and rebuilds the two dropdowns to their original code.
...
menu_str += '<div class="txtHint1" id="txtHint1" name="txtHint1"><b> WORDING_TXTHINT1 </b></div></div>';
//added a new param to showSubCat1 to hold the original catid of the page being displayed (so that a Clear will display the correct subcategories of the page they are in)
var menu_str_pre = '<span id="mn_subcat_container"> <form action=""><table id="mn_subcat_table"><tr><td style="width:45%;"><select name="subCat1" onchange="updategoButton(this.value,\'false\', \'' + main_cat_nonce + '\',\''+original_cat_id+'\'), showSubCat1(this.value,\''+ main_cat_nonce +'\',\''+original_cat_id+'\')"><option value="">' + wording_ajax_menu1 + '</option> ';
                                                                                                                                                                                                         
menu_str = menu_str_pre + menu_str; 
document.getElementById("column-left").innerHTML = menu_str; 
var region_str = '<span id="mn_location_container"><form action=""> <table id="mn_location_table"><tr><td><select name="tregional_num" onchange="updategoButton(';
region_str += "'n:" + category_id + ":\"catname\"', this.value, \'" + main_cat_nonce + '\',\''+original_cat_id+'\'), showSubLoc1(this.value,\''+ main_cat_nonce+'\')"><option value="">'+wording_ajax_regional_menu1+'</option> ';
region_str += "	<option value='y:2566:Africa'>Africa</option>	<option value='y:2567:America - Central'>America - Central</option>		<option value='y:2568:America - North'>America - North</option>		<option value='y:2569:America - South'>America - South</option>		<option value='y:2572:Asia'>Asia</option>		<option value='y:2573:Australia/Oceania'>Australia\/Oceania</option>		<option value='y:2756:Caribbean'>Caribbean</option>		<option value='y:2575:Europe'>Europe</option>		<option value='y:2740:Middle East'>Middle East</option>";
region_str += '<input type="hidden" id="regional_name" name="regional_name" class ="regional_name" value=""><input type="hidden" id="tregional_num" name="tregional_num" class ="tregional_num" value=""></select><br><div id="locHint1" name="locHint1"><b>Smaller Regions Available After Selection.</b></td></tr></table><input type="hidden" id="regional_name" name="regional_name" class ="regional_name" value=""><input type="hidden" id="tregional_num" name="tregional_num" class ="tregional_num" value=""></form></span>';
region_str += ' <div id="locHint1" name="locHint1"><b>Smaller Regions Available After Selection.</b></div>';
window.alert('region_str = '+region_str);
document.getElementById("column-right").innerHTML = region_str; 

if(document.contains(document.getElementById('txtHint1')) == true){
document.getElementById('txtHint1').innerHTML= '<b>More Subcategories Available After Selection</b>';
}
if(document.contains(document.getElementById('txtHint2')) == true){
document.getElementById('txtHint2').innerHTML= '<b></b>';
}
if(document.contains(document.getElementById('txtHint3')) == true){
document.getElementById('txtHint3').innerHTML= '<b></b>';
}
if(document.contains(document.getElementById('txtHint4')) == true){
document.getElementById('txtHint4').innerHTML= '<b></b>';
}
if(document.contains(document.getElementById('txtHint5')) == true){
document.getElementById('txtHint5').innerHTML= '<b></b>';
}
if(document.contains(document.getElementById('txtHint6')) == true){
document.getElementById('txtHint6').innerHTML= '<b></b>';
}
if(document.contains(document.getElementById('locHint1')) == true){
document.getElementById('locHint1').innerHTML= '<b>Smaller Regions Available After Selection.</b>';
}
if(document.contains(document.getElementById('locHint2')) == true){
document.getElementById('locHint2').innerHTML= '<b></b>';
}
if(document.contains(document.getElementById('locHint3')) == true){
document.getElementById('locHint3').innerHTML= '<b></b>';
}
if(document.contains(document.getElementById('locHint4')) == true){
document.getElementById('locHint4').innerHTML= '<b></b>';
}
document.getElementById('column-center').innerHTML= '<p id="goLink" name="goLink" class="goLink">&nbsp;</p>';
 }
}
 xmlhttp.open("GET","/wp-content/plugins/manna-network/getsubcatdelete.php?category_id="+original_cat_id+"&main_cat_nonce='"+main_cat_nonce+"'",true);
 xmlhttp.send();
...
function showSubLoc1(str, main_cat_nonce) {
var myarr = str.split(":");
 ...
 /*if (str=="") {
  ...
  xmlhttp.onreadystatechange=function() {
    ...
    if (this.readyState==4 && this.status==200) {
      document.getElementById("locHint1").innerHTML=this.responseText+'<div id="locHint2" name="locHint2"><b>'+still_more_locs_cont+'</b></div>';
    }
  }
document.getElementById("tregional_num").value = myarr[1];
document.getElementById("regional_name").value = myarr[2];
  xmlhttp.open("GET","/wp-content/plugins/manna-network/getsubloc1.php?tregional_num="+myarr[1]+"&main_cat_nonce='"+main_cat_nonce+"'",true);
  xmlhttp.send();
}



function showSubLoc2(str, main_cat_nonce) {
...
document.getElementById("tregional_num").value = myarr[1];
document.getElementById("regional_name").value = myarr[2];
  xmlhttp.open("GET","/wp-content/plugins/manna-network/getsubloc2.php?tregional_num="+myarr[1]+"&main_cat_nonce='"+main_cat_nonce+"'",true);
  xmlhttp.send();
}


function showSubLoc3(str, main_cat_nonce) {
var myarr = str.split(":");
 if (str=="") {
...
document.getElementById("regional_name").value = myarr[2];
  ...
  xmlhttp.open("GET","/wp-content/plugins/manna-network/getsubloc3.php?tregional_num="+myarr[1]+"&main_cat_nonce='"+main_cat_nonce+"'",true);
  xmlhttp.send();
}

function showSubLoc4(str, main_cat_nonce) {
var myarr = str.split(":");
